...
/*
/* ta-side-custombox-callback.php: ( Latest Version 2.04 2017.02.24 )
/*
...
/* File Version 2.02 2016.06.28: サイドバーフリーエリアのh3 classからfirst-title削除 by Kotaro Kashiwamura.
/* File Version 2.03 2016.08.27: 通常表示（コンテンツをPC時に表示するか）の追加
/*                               順序番号の位置修正 by Kotaro Kashiwamura.
/* File Version 2.04 2017.02.24: サイドバーメニュー追加、体裁調整 by Kotaro Kashiwamura.
/*
/*******************************************************************************************************/
...
wp_nonce_field( 'ta_nonce_key', 'ta_custombox' ); ?>
  ...
  <table class="ta-setting-table">
    <?php ta_sidebar_custombox_message_disp(); ?>
    <!-- サイドバーフリーエリア -->
    <tr>
      <th id="ta_freearea_basical_setting_title" class="big-title"><a href="JavaScript:void(0);">サイドバーフリーエリア</a></th>
      <td>
        <div id="ta_freearea_basical_setting_disp" class="init-nodisp">
          <?php ta_sidebar_top_margin_setting( 'ta_post', get_the_ID() ); ?>
          ...
          <div id="ta_post_freearea_pro_disp" class="pro-disp init-nodisp">
            ...
            <?php ta_factor_selection_process( 'ta_post_freearea_title_factor', 'postmeta', get_the_ID(), 'common', 'サイドバー' ); ?>
          </div><!-- end of #ta_post_freearea_pro_disp -->
          <span class="fixed-space"></span>
        </div><!-- end of #ta_freearea_basical_setting_disp -->
      </td>
    </tr>
    <!-- サイドバーフリーエリア・バナー画像・付随文 -->
    <tr>
      <th id="ta_freearea_banner_imgtxt_title" class="big-title"><a href="JavaScript:void(0);">サイドバーフリーエリア<br>バナー画像・付随文</a></th>
      <td>
            ...
            <table class="ta-2contents-table">
              <tr>
                <td>
                  ...
                  <?php ta_simple_input( 'ta_post_sidebar_freearea_img_width', '％', 'short_box', 'postmeta', get_the_ID() ); ?>
                </td>
                <td>
    ...
    <!-- サイドバーメニュー -->
    <tr>
      <th id="ta_freearea_side_menu_title" class="big-title"><a href="JavaScript:void(0);">サイドバーメニュー</a></th>
      <td>
        <div id="ta_freearea_side_menu_disp" class="init-nodisp">
          <span class="fixed-space"></span>
          <p>※ TAサイドバーメニュー（子孫展開無しの縦型）はWordPress標準のメニュー機能を使用しています。有効にするためには関連付けが必要です。
          ...
          <span class="fixed-space"></span>
          <?php $side_menu_num = ta_read_post( get_the_ID(), 'ta_post_freearea_menu_num' ); ?>
          <p><input type="radio" name="ta_post_freearea_menu_num" value="none" <?php if ( "none" == $side_menu_num ) echo 'checked="checked"' ?>> 使用しない</p>
<?php
for ( $i = 1; $i <= 10; $i++ ) { ?>
          <p class="side-menu-option<?php echo $i; ?>"><input type="radio" name="ta_post_freearea_menu_num" value="<?php echo $i; ?>" <?php if ( $i == $side_menu_num ) echo 'checked="checked"' ?>> TAサイドバーメニュー#<?php echo $i; ?></p>
<?php
}
if ( ! TA_HIEND_PI && 'none' != $side_menu_num && 1 != $side_menu_num ) { ?>
          <p>※ TAサイドバーメニュー#2～#10は選択可能ですが、<span style="color:#007fff;">プラグイン『HighEnd』</span>使用時に有効になります。</p>
<?php
} ?>
          <span class="fixed-space"></span>
        </div><!-- end of #ta_freearea_side_menu_disp -->
      </td>
    </tr>
<?php
if ( ! TA_HIEND_PI ) { ?>
...
<?php
    ...
    <tr>
      ...
      <th id="ta_freearea_frame_border_title" class="big-title"><a href="JavaScript:void(0);">サイドバーフリーエリア<br>コンテンツ枠</a></th>
      <td>
...
<?php
if ( TA_HIEND_PI ) { ?>
      <!-- サイドバーフリーエリア・非表示ページ（このコンテンツを非表示にしたいページの選択）-->
      <th id="ta_freearea_no_display_title" class="big-title"><a href="JavaScript:void(0);">サイドバーフリーエリア<br>非表示ページ</a></th>
      <td>
        <div id="ta_freearea_no_display_disp" class="init-nodisp">
          <span class="fixed-space"></span>
          <p>※ このコンテンツを非表示にしたいページの選択です</p>
          <span class="fixed-space"></span>
          <?php ta_freearea_no_display(); ?>
          <span class="fixed-space"></span>
        </div><!-- end of #ta_freearea_no_display_disp -->
      </td>
<?php
} else { ?>
      <th class="no-ta-thm001highend">サイドバーフリーエリア<br>非表示ページ</th>
      <td></td>
<?php
} ?>
    </tr>
    <!-- サイドバーフリーエリア・レスポンシブ表示（このコンテンツをレスポンシブ有効時に表示するか）-->
    <tr>
      <th id="ta_freearea_responsible_disp_title" class="big-title"><a href="JavaScript:void(0);">サイドバーフリーエリア<br>レスポンシブ表示</a></th>
      <td>
    ...
    <!-- サイドバーフリーエリア・通常表示（このコンテンツをPC時に表示するか）-->
    <tr>
      <th id="ta_post_freearea_normal_disp_title" class="big-title"><a href="JavaScript:void(0);">サイドバーフリーエリア<br>通常表示</a></th>
      <td>
